...
class GitHubStorage:
    ...
    def get_connection():
        """尝试连接 GitHub"""
        if not HAS_GITHUB: return None
        
        # --- 用户特定配置 (Hardcoded Fallback) ---
        # 注意：包含 Token 的代码不应公开分享到公共仓库。
        # 此配置用于本地直接运行，无需 secrets.toml 文件。
        DEFAULT_REPO = "shumao469/stroke-data-storage"
        DEFAULT_TOKEN = "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"

        try:
            # 1. 优先尝试从安全配置 (st.secrets) 获取
            # 这通常用于 Streamlit Cloud 部署环境
            try:
                token = st.secrets.get("GITHUB_TOKEN", DEFAULT_TOKEN)
                repo_name = st.secrets.get("REPO_NAME", DEFAULT_REPO)
            except (FileNotFoundError, AttributeError):
                # 如果本地没有 secrets.toml，则使用硬编码的默认值
                token = DEFAULT_TOKEN
    ...
    def get_cohort_count():
        ...
        if repo:
            ...
            # 2. 如果没有连接 GitHub，使用 Session State 模拟新增
    ...
    def save_report(patient_data):
        ...
        if repo:
            try:
                # 检查 reports 文件夹是否存在，理论上 create_file 会自动处理路径，但在某些 API 版本需注意
                # 直接尝试创建文件
            ...
            # 模拟网络延迟
            time.sleep(1) 
            return True, "Simulated Save (Local Mode) - Secrets not configured."
...
def main():
    ...
    st.sidebar.info("v5.2 | GitHub Integration")
