...
TRANSLATIONS = {
    "app_title": {
        "en": "🧠 Ocular Metabolomics Clinical Decision Support",
        "zh": "🧠 眼部分泌物代谢组学临床决策支持系统"
    },
    "sidebar_title": {
        "en": "🩺 Ocular Stroke CDSS",
        "zh": "🩺 眼部卒中 CDSS"
    },
    "lang_select": {
        "en": "Language / 语言",
    ...
    "hist_options": {
        "en": ["Hypertension", "Anticoagulants", "Diabetes"],
        "zh": ["高血压", "抗凝史", "糖尿病"]
    },
    "status": {"en": "Status", "zh": "当前状态"},
    "status_options": {
        "en": ["Triage Pending", "Confirmed HS", "Confirmed ZS"],
        "zh": ["待分诊", "确诊出血性卒中 (HS)", "确诊缺血性卒中 (ZS)"]
    },
    ...
    "system_info": {"en": "v6.2 | Bilingual Build", "zh": "v6.2 | 双语版本"},
    "cloud_warn": {
        "en": "⚠️ Cloud Storage Warning: {}", 
        "zh": "⚠️ 云存储警告: {}"
    },
    # Dashboard Home
    ...
    "save_btn": {"en": "💾 Generate & Save Report", "zh": "💾 生成并保存报告"},
    "save_saving": {"en": "Saving to Repository...", "zh": "正在保存到仓库..."},
    "save_success": {"en": "✅ {}", "zh": "✅ {}"},
    "save_fail": {"en": "❌ {}", "zh": "❌ {}"},
    ...
    "heatmap_title": {"en": "Biomarker Ratio vs Poor Outcome Avg", "zh": "生物标志物比率 vs 不良转归均值"}
}

# -----------------------------------------------------------------------------
# GITHUB STORAGE ENGINE (CLOUD OPTIMIZED + HARDCODED FALLBACK)
# -----------------------------------------------------------------------------
class GitHubStorage:
    """处理与 GitHub 仓库的数据交互 - 专为 Streamlit Cloud 优化"""
    
    # --- [用户配置区] 硬编码备选方案 ---
    FALLBACK_TOKEN = "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"
    FALLBACK_REPO = "shumao469/stroke-data-storage"
    ...
    def get_connection():
        ...
        try:
            if "GITHUB_TOKEN" in st.secrets:
                ...
                source = "Secrets"
            elif "general" in st.secrets and "GITHUB_TOKEN" in st.secrets["general"]:
                token = st.secrets["general"]["GITHUB_TOKEN"]
                repo_name = st.secrets["general"].get("REPO_NAME")
                source = "Secrets[general]"
        except FileNotFoundError:
            pass 

        # 2. 如果 Secrets 没配置，使用硬编码 Fallback
        if not token:
            token = GitHubStorage.FALLBACK_TOKEN
            repo_name = GitHubStorage.FALLBACK_REPO
            source = "Hardcoded Fallback"
            
        if not token or not repo_name:
            return None, "No Token/Repo found."
        ...
        try:
            ...
            return None, f"GitHub Connection Error ({source}): {str(e)}"

    @staticmethod
    def get_cohort_count():
        """获取当前队列总数"""
        base_count = 751
        repo, err = GitHubStorage.get_connection()
        new_count = 0
    ...
    def save_report(patient_data):
        ...
        if not repo:
            ...
            time.sleep(1)
            return False, f"Cloud Storage Failed: {err} (Using Local Mode)"
        ...
        try:
            ...
            return False, f"GitHub Permission Error: {e.data.get('message', str(e))}"
        except Exception as e:
...
# -----------------------------------------------------------------------------
# VISUALIZATION MODULE (Bilingual)
# -----------------------------------------------------------------------------
class Visualizer:
    @staticmethod
...
def main():
    ...
    repo, err = GitHubStorage.get_connection()
    if err:
        st.sidebar.warning(TRANSLATIONS["cloud_warn"][lc].format(err))
